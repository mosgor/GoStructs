// Package minHeap contains an implementation of the Heap data structure
// which elements are sorted from the smallest to the biggest and an implementation of methods
...
//
// Also, it implements Heapify function that can convert any slice to MinHeap.
package minHeap
...
// MinHeap is a sorted in increasing order implementation of the Heap data structure based on a slice.
...
// MinHeap takes generic type T, that can be every int, uint, float or string type.
type MinHeap[T values] []T
...
func (h *MinHeap[T]) Insert(k T) error {
	if h == nil {
...
// Extract method removes the root and concurrently the smallest element from the heap and returns it.
...
// Min method returns the root and concurrently the smallest element from the heap.
// This method returns a type T default value if the heap is empty or if the receiver is nil.
...
func Heapify[T values](arr []T) MinHeap[T] {
